// lib/handlers/auth-school-register.js
...
async function findMatchingLead(conn, email, phoneE164) {
  ...
  const patLocal = phoneDigits.startsWith("62")
    ? phoneDigits.slice(2)
    : phoneDigits;
...
async function ensureOppRecordType(conn) {
  const r = await conn.query(`
    SELECT Id
    FROM RecordType
    WHERE SobjectType = 'Opportunity' AND DeveloperName = 'School'
    LIMIT 1
  `);
  return r.records?.[0]?.Id || null;
}
...
async function createPersonAccount(
  ...
  if (!res.success) {
    throw new Error(
      (res.errors && res.errors.join(", ")) || "Failed to create Account"
    );
  }
  return res.id;
}
...
async function ensureOpportunity(conn, accountId, nameHint) {
  ...
  if (!res.success) {
    throw new Error(
      (res.errors && res.errors.join(", ")) || "Failed to create Opportunity"
    );
  }
  return res.id;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (lead) {
      if (!lead.IsConverted) {
        ...
        if (lead?.IsConverted) {
          ...
          try {
            const uniRT = await ensureOppRecordType(conn); // already defined in this file
        ...
        if (sameEmail) {
          ...
          try {
            const uniRT = await ensureOppRecordType(conn);
            if (opportunityId && uniRT) {
              await conn.sobject("Opportunity").update({
                Id: opportunityId,
                RecordTypeId: uniRT,
    ...
    if (!opportunityId) {
      const oppName = `${firstName} ${lastName || ""}`.trim();
