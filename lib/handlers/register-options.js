  ...
  try {
    ...
    if (method === 'GET') {
      ...
      if (type === 'campuses' || type === 'campus') {
        ...
        // 1) Primary: Campus__c (simple, permissive)
        try {
          ...
          const q = await conn.query(`
            ...
            LIMIT 200
          `);
          const rows = (q.records || []).map(r => ({ Id: r.Id, Name: r.Name }));
          if (rows.length) return ok(res, { records: rows, source: 'Campus__c' });
        } catch (e1) {
        ...
        try {
          ...
          return ok(res, { records: rows, source: 'Faculty_Campus__c', errors });
        } catch (e2) {
      ...
      if (type === 'pricing') {
        ...
        return ok(res, {
          bspId: batchId,       // optional association with the active batch
          bspName: batchName,   // label to show which batch is active
          bookingPrice
        });
      }
