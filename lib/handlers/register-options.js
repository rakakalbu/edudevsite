...
const CACHE_SECS = 60; // edge cache hint

// ---- Reuse a single SF connection per server instance (cuts API usage)
let _conn = null;
let _lastLoginAt = 0;
...
module.exports = async (req, res) => {
  try {
    ...
    if (method === 'GET') {
      ...
      const dateStr    = (query.date ?? '').trim();     // for masterBatch/pricing
      ...
      if (type === 'campuses' || type === 'campus') {
        ...
        try {
          ...
          errors.push('Campus__c: ' + (e1?.message || String(e1)));
        }

        // 2) Fallback: distinct Campus via Faculty_Campus__c → Campus__c
        try {
          const q = await conn.query(`
        ...
        // 3) Last resort: distinct names via Account.Master_School__c (keeps UI usable)
        try {
          const q = await conn.query(`
      ...
      if (type === 'intakes' || type === 'intake') {
        ...
        try {
          // If your schema doesn’t link intake to campus, remove the WHERE
          const q = await conn.query(`
            SELECT Id, Name
          ...
          // graceful fallback with dynamic years
      ...
      if (type === 'programs' || type === 'program') {
        ...
        try {
          ...
          // SP_Faculty_Campus limited by chosen intake (via SP_Intake)
          ...
          return ok(res, { records: [], source: 'error', errors });
        }
      }

      // ---------- MASTER BATCH (for intake + date) ----------
      if (type === 'masterbatch') {
        if (!intakeId) return fail(res, 400, 'intakeId wajib diisi');
      ...
      // ---------- BSP (echo or lookup, depending on org) ----------
      if (type === 'bsp') {
        ...
        // If there is a real BSP object, you can look it up here. Otherwise echo.
        return ok(res, { id: `${masterBatchId}::${studyProgramId}`, name: 'BSP', masterBatchId, studyProgramId });
      }

      // ---------- PRICING (BSP + Booking_Form_Price__c) ----------
      if (type === 'pricing') {
        if (!intakeId || !studyProg) return fail(res, 400, 'intakeId & studyProgramId wajib diisi');
        const when = dateStr || new Date().toISOString().slice(0, 10);
        const conn = await getConn(process.env);
        setCache(res, 30); // pricing can change—cache shorter

        // 1) Active batch for intake + date
        ...
        // 2) Junction price (adjust object/field if your org uses different names)
      ...
      // ---------- SCHOOLS (broader search with fallbacks) ----------
    ...
    if (method === 'POST') {
      ...
      if (body?.action === 'saveReg') {
        ...
        await conn.sobject('Opportunity').update({
          ...
          // If you have a BSP field, uncomment:
          // BSP__c: bspId || null
        });
