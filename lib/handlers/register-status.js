...
// Returns all details required to hydrate the wizard.
...
module.exports = async (req, res) => {
  try {
    ...
    if (!isSfId(opportunityId)) return send(res, 400, { success: false, message: 'Invalid opportunityId' });

    const conn = await login(process.env);

    // Pull Opp + Account + selections; also grab SP price.
    ...
    // Active batch (BSP) for label only
    let bspName = null;
    try {
      ...
      console.warn('register-status: batch lookup failed:', e?.message || e);
    }
