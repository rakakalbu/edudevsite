...
async function queryWithFallback(conn, oppId) {
  ...
  try {
    ...
    // Try tiny follow-up for just the fields we need
    let add = {};
    try {
      const extra = await conn.query(`
        SELECT Id, Graduation_Year__c, Draft_Sekolah__c, Draft_NPSN__c, Master_School__c, Master_School__r.Name
        FROM Opportunity
        WHERE Id = '${oppId}'
        LIMIT 1
      `);
      const r = extra.records?.[0];
      if (r) add = {
        Graduation_Year__c: r.Graduation_Year__c ?? null,
        Draft_Sekolah__c:   r.Draft_Sekolah__c   ?? null,
        Draft_NPSN__c:      r.Draft_NPSN__c      ?? null,
        Master_School__c:   r.Master_School__c   ?? null,
        Master_School__r:   { Name: r.Master_School__r?.Name ?? null }
      };
    } catch { /* still fine */ }

    return { record: { ...rec, ...add }, mode: 'minimal', warn: String(e?.message || e) };
  }
}
...
module.exports = async (req, res) => {
  try {
    ...
    // School block (works in both modes now)
