...
// GET  /api/salesforce-query?type=school|intake|major&term=...&campusId=... -> lookup helpers (from promo site)
...
async function handlePost(req, res) {
  ...
  if (typeof webStage !== "number")
    return send(res, 400, {
      success: false,
      message: "webStage (number) required",
    });
  ...
  try {
    ...
    await conn
      .sobject("Opportunity")
      .update({ Id: opportunityId, Web_Stage__c: webStage });
...
async function handleGet(req, res) {
  ...
  if (!type)
    return send(res, 400, { success: false, message: "type wajib diisi" });
  ...
  try {
    ...
    return send(res, 500, {
      success: false,
      message:
        e.code === "ENV_MISSING" ? e.message : "Gagal login ke Salesforce",
  ...
  try {
    ...
    if (type === "school_own") {
      try {
        ...
        const needFallback =
          /INVALID_TYPE|sObject type .* is not supported|No such column/i.test(
            msg
          );
        if (!needFallback) {
        ...
        try {
          ...
          return send(res, 500, {
            success: false,
            message: "Gagal query fallback Campus",
    ...
    if (type === "program") {
      ...
      try {
        ...
        return send(res, 500, {
          success: false,
          message: "Gagal query Study Program",
          error: msg,
        });
      }
    }
    ...
    return send(res, 400, {
      success: false,
      message: `type tidak dikenali: ${type}`,
    });
  } catch (e) {
...
module.exports = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: String((e && e.message) || e) });
  }
};
