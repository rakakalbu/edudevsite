...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (lead && !lead.IsConverted) {
      ...
      const conv = await conn.sobject('Lead').convert({
        leadId: lead.Id,
      ...
      if (!conv || !conv.success) throw new Error(conv?.errors?.join(', ') || 'Lead convert failed');

      // Normalize some fields after conversion
      if (conv.accountId) {
    ...
    // If converted already, reuse
    if (lead && lead.IsConverted) {
      // Also force StageName/book phone on person account if needed
    ...
    // No lead? (Keep existing fallback behavior if you rely on this endpoint)
