...
async function createPersonAccount(conn, { firstName, lastName, email, phoneE164, passwordHash }) {
  ...
  const acc = {
    ...
    PersonMobilePhone: phoneE164, // keep both, some page layouts show either
...
/**
 * Look for the newest Opportunity under an Account that was created very recently.
 * This is used right after a Lead conversion to avoid creating a duplicate
 * when your Apex/Flow creates an Opportunity a moment later.
 */
async function findVeryRecentOpportunity(conn, accountId, maxAgeMs = 5 * 60 * 1000) {
  ...
  const q = await conn.query(`
    ...
    LIMIT 1
  `);
  const opp = q.records?.[0] || null;
  if (!opp) return null;
  try {
    const age = Date.now() - new Date(opp.CreatedDate).getTime();
    if (age >= 0 && age <= maxAgeMs) return opp.Id;
  } catch {}
  return null;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (lead) {
      if (!lead.IsConverted) {
        ...
        if (lead?.IsConverted) {
          ...
          if (accountId) {
            // keep the phone populated on the Person Account
          ...
          // === NEW: guard against duplicate Opp creation ===
          if (!opportunityId && accountId) {
            // Give your async flow a brief chance, then look for a very recent Opp
            await wait(600); // a quick nudge; harmless if no async Opp is created
            opportunityId = await findVeryRecentOpportunity(conn, accountId);
          }
        }
      } else {
        // Already converted
    ...
    if (!opportunityId) {
      ...
      // make sure stage/close date are as expected
