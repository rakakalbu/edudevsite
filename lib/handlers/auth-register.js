...
async function createPersonAccount(conn, { firstName, lastName, email, phoneE164, passwordHash }) {
  ...
  const acc = {
    ...
    LastName : lastName || '—',
    PersonEmail: email,
    // Put number into Account.Phone aggregate (Person Accounts UI shows Phone(2))
    PersonHomePhone: phoneE164,
    // Store hashed password into your Account custom field
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // NEW: ensure we store a hash, even if client sent plaintext
    ...
    if (lead) {
      if (!lead.IsConverted) {
        // Clear Company to force Person Account, set password on Lead, trigger Apex convert
        await conn.sobject('Lead').update({
          ...
          Email    : email,
          Phone    : phoneE164,
          Company  : null,               // make it Person Account on convert
          Password__c: passwordHash,     // hashed value
          Is_Convert__c: true
        ...
        if (lead?.IsConverted) {
          ...
          // Ensure Account has the hashed password (in case mapping is missing)
    ...
    // If still missing, create directly
    if (!accountId) {
