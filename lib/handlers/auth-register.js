...
/** POST email+password to your external login service (non-blocking). */
async function provisionExternalLogin(email, passwordHash) {
  try {
    if (!email || !passwordHash) return; // nothing to send
    await fetch('https://salesforceedupakcage.vercel.app/api/register', {
      method : 'POST',
      headers: { 'Content-Type': 'application/json' },
      body   : JSON.stringify({ email, password: passwordHash })
    }).catch(() => {}); // swallow fetch-level network errors
  } catch (e) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Always store/send a hash (if client already sends a hash, we keep it)
    const passwordHash = toPasswordHash(password);
    ...
    // Try Lead-first path (conversion via Is_Convert__c)
    ...
    if (lead) {
      if (!lead.IsConverted) {
        await conn.sobject('Lead').update({
          ...
          Password__c: passwordHash,     // hashed
          Is_Convert__c: true
        ...
        if (lead?.IsConverted) {
          ...
          // Ensure Account has the hashed password
    ...
    // If still missing, create directly (Person Account)
    if (!accountId) {
    ...
    // 🔔 NEW: call your external login API (non-blocking, failure-safe)
    provisionExternalLogin(email, passwordHash).catch(()=>{});
