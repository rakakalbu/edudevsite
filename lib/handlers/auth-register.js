...
async function createPersonAccount(conn, { firstName, lastName, email, phoneE164, passwordHash }) {
  ...
  const acc = {
    ...
    LastName : lastName || '—',
    PersonEmail: email,
    // ✅ populate ALL relevant phone slots on Person Account
    Phone: phoneE164,
...
async function ensureOpportunity(conn, accountId, nameHint) {
  ...
  const name = (nameHint && nameHint.trim()) ? `${nameHint}/REG/${ts}` : `${todayISO()}/REG/${ts}`;
  ...
  try {
    ...
    if (lead) {
      ...
      if (canReuseLead && !lead.IsConverted) {
        // Convert via Apex flag; DO NOT overwrite Lead names
      ...
      if (canReuseLead && lead?.IsConverted) {
        ...
        if (accountId) {
          // ✅ make sure all phone slots are populated on the converted Account
          const accUpd = {
            Id: accountId,
            Password__c: passwordHash || null,
            Phone: phoneE164,
          };
          await conn.sobject('Account').update(accUpd).catch(()=>{});
      ...
      // if names don't match, ignore the lead and create fresh Account below.
    }
