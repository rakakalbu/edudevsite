...
async function createPersonAccount(conn, { firstName, lastName, email, phoneE164, passwordPlain }) {
  ...
  // Populate BOTH Phone and PersonMobilePhone so the header shows "Phone"
  // while any logic that expects mobile still works.
  const acc = {
    FirstName: firstName,
    LastName : lastName || '—',
    PersonEmail: email,
    Phone: phoneE164,              // <-- primary phone on Account header
    PersonMobilePhone: phoneE164,  // <-- keep mobile in sync
  };

  // Store the hashed password in your custom field
  if (hash) acc.Password__c = hash;

  if (rtId) acc.RecordTypeId = rtId;

  const res = await conn.sobject('Account').create(acc);
  if (!res.success) throw new Error((res.errors && res.errors.join(', ')) || 'Failed to create Account');

  // OPTIONAL: mirror to Contact here if you keep a Contact.Password__c field
  return res.id;
}
...
async function ensureOpportunity(conn, accountId, nameHint) {
  ...
  const ins = {
    ...
    AccountId: accountId,
    StageName: 'Booking Form',      // <<< CHANGED: initial stage
    CloseDate: todayISO()
  };
  if (rtId) ins.RecordTypeId = rtId;

  const res = await conn.sobject('Opportunity').create(ins);
  if (!res.success) throw new Error((res.errors && res.errors.join(', ')) || 'Failed to create Opportunity');
  return res.id;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Ensure an Opportunity (now also starts at "Booking Form")
    if (!opportunityId) {
