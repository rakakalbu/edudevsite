...
async function getOppRT(conn) {
  const HARD = '012gL000002NZITQA4'; // fallback if valid in your org
  try {
...
async function setLeadConvertFlagBestEffort(conn, leadId) {
  try {
    ...
    try {
      ...
      // swallow: we will still convert via API; return info for debugging
      return 'flag-not-set';
    }
  }
}
...
async function leadConvertAPI(conn, leadId) {
  ...
  if (!r.success) {
    ...
    // Common noisy org error we can recover from:
    if (/converted lead/i.test(msg) || /update converted lead/i.test(msg)) {
      // Re-fetch; maybe it already converted in parallel
...
export default async function handler(req, res) {
  try {
    ...
    const {
      ...
      // passthrough (accepted but unused here)
    ...
    if (lead) {
      ...
      // 2) Hybrid conversion:
      // (a) best-effort set your checkbox for internal flows/logs
      const flagField = await setLeadConvertFlagBestEffort(conn, lead.Id);

      // (b) convert via API (more reliable / separate transaction)
      let triple = null;
      try {
        triple = await leadConvertAPI(conn, lead.Id);
      } catch (err) {
        // If org automation still racing, try a short poll as last resort
        const polled = await pollConvertedTriple(conn, lead.Id, 8000, 600);
        if (polled) triple = polled;
        else throw err; // surface real error to client
      }

      // (c) If no opp id was provided, pick latest on the account
      ...
      return res.json({
        success: true,
        source: `lead-converted:${flagField}`,
        ...triple
      });
    }

    // 3) No lead → create Person Account + Contact + Opp
    const created = await createDirect(conn, { firstName, lastName, email, phone });
    return res.json({ success: true, ...created });

  } catch (e) {
    // Important: do not leak Salesforce internal stack; pass clean message up
