...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Always update graduation year on Opportunity
    ...
    if (masterSchoolId) {
      ...
      oppUpd.Draft_School__c = null;
      oppUpd.Draft_NPSN__c   = null;
    } else {
      // MANUAL MODE (NPSN optional)
      const onlyDigits = String(draftNpsn || '').replace(/\D/g, '');
      if (!draftSchool) throw new Error('Nama sekolah wajib diisi');

      oppUpd.Draft_School__c = draftSchool;
      oppUpd.Draft_NPSN__c   = onlyDigits || null;
    }
    ...
    console.error('register-save-educ ERR:', err);
