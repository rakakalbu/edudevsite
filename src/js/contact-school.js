(function () {
  ...
  function confirmPreview(data) {
    const html = `
      <div style="text-align:left">
        <div><strong>Nama:</strong> ${data.firstName} ${
      data.lastName || ""
    }</div>
        <div><strong>Email:</strong> ${data.email}</div>
        <div><strong>Phone:</strong> ${data.phone}</div>
        <div><strong>Jenjang:</strong> ${data.educationLevel}</div>
        <div><strong>Sekolah Metro:</strong> ${
          data.metroSchoolName || "-"
        }</div>
  ...
  async function submitForm(e) {
    ...
    const educationLevel = document.querySelector(
    ...
    try {
      ...
      if (json?.success && json.records?.length) {
        ...
        json.records.forEach((school) => {
          ...
          div.innerHTML = `
            <img src="https://picsum.photos/200?random=${encodeURIComponent(
              school.Id
            )}" alt="${school.Name}">
          ...
          div.addEventListener("click", () => {
            ...
            list
              .querySelectorAll(".metro-school-card")
