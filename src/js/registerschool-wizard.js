...
(() => {
  ...
  async function api(url, opts) {
    ...
    try {
      ...
      throw new Error(
        t?.slice(0, 400) || "Server mengembalikan respons non-JSON"
      );
    }
  ...
  const rupiah = (n) =>
    n == null || isNaN(n) ? "Rp -" : "Rp " + Number(n).toLocaleString("id-ID");
  ...
  const S = {
    get opp() {
      return localStorage.getItem(K("opp")) || "";
    },
    set opp(v) {
      localStorage.setItem(K("opp"), v || "");
    },
    get acc() {
      return localStorage.getItem(K("acc")) || "";
    },
    set acc(v) {
      localStorage.setItem(K("acc"), v || "");
    },
    set pemohon(o) {
      localStorage.setItem(K("pemohon"), JSON.stringify(o || {}));
    },
    get pemohon() {
      try {
        return JSON.parse(localStorage.getItem(K("pemohon")) || "{}");
      } catch {
        return {};
      }
    },
    set reg(o) {
      localStorage.setItem(K("reg"), JSON.stringify(o || {}));
    },
    get reg() {
      try {
        return JSON.parse(localStorage.getItem(K("reg")) || "{}");
      } catch {
        return {};
      }
    },
    set sekolah(o) {
      localStorage.setItem(K("sekolah"), JSON.stringify(o || {}));
    },
    get sekolah() {
      try {
        return JSON.parse(localStorage.getItem(K("sekolah")) || "{}");
      } catch {
        return {};
      }
    },
  ...
  function clearAllRegState() {
    try {
      ["opp", "acc", "pemohon", "reg", "sekolah"].forEach((k) =>
  ...
  function setStep(n) {
    $$(".form-step").forEach(
      (s) => (s.style.display = s.dataset.step === String(n) ? "" : "none")
    );
    updateProgress(n);
  ...
  $("#formLogin")?.addEventListener("submit", async (e) => {
    ...
    if (!emailOk(email) || !password) {
      msg.textContent = "Masukkan email dan kata sandi yang valid.";
  ...
  $("#formStep1").addEventListener("submit", async (e) => {
    ...
    if (pass !== pass2) {
      msg.textContent = "Ulangi kata sandi tidak cocok.";
      msg.style.display = "block";
      return;
    }
    ...
    try {
      ...
      $("#opptyIdLabel").textContent = j.opportunityId;
      ...
      closeLoading();
      toastOk("Akun disiapkan. Pilih program.");
      setStep(2);
      updateStage(2);
      loadStep2Options();
    } catch (err) {
  ...
  async function loadIntakes(campusId) {
    ...
    recs.forEach(
      (x) => (sel.innerHTML += `<option value="${x.Id}">${x.Name}</option>`)
    );
  }
  ...
  $("#formStep2_Prefs").addEventListener("submit", async (e) => {
    ...
    try {
      ...
      closeLoading();
      toastOk("Preferensi studi tersimpan. Lanjut ke pembayaran.");
      setStep(3);
      updateStage(3);
    } catch (err) {
      closeLoading();
  ...
  $("#formStep3_Payment").addEventListener("submit", async (e) => {
    ...
    msg.style.display = "none";
    if (!oppId) {
      showError("Opportunity belum tersedia.");
      return;
    }
    if (!file) {
      msg.textContent = "Pilih file bukti pembayaran.";
      msg.style.display = "block";
      return;
    }
    if (file.size > 1024 * 1024) {
      msg.textContent = "Maksimal 1MB.";
      msg.style.display = "block";
      return;
    }
    const allowed = ["application/pdf", "image/png", "image/jpeg"];
    if (file.type && !allowed.includes(file.type)) {
      msg.textContent = "Format harus PDF/PNG/JPG.";
      msg.style.display = "block";
      return;
    }
    ...
    try {
      ...
      closeLoading();
      toastOk("Bukti pembayaran berhasil diupload.");
      setStep(4);
      updateStage(4);
      populateYears();
      initStep4();
  ...
  function initStep4() {
    ...
    const setManualMode = (manual) => {
      ...
      autoWrap?.querySelector?.(".suggest-box") &&
        (autoWrap.querySelector(".suggest-box").style.display = manual
          ? "none"
          : "");
      if (manual) {
    ...
    cbNotFound?.addEventListener("change", () =>
    ...
    const onType = debounce(async () => {
      ...
      try {
        const j = await api(
          `/api/register-options?type=sekolah&term=${encodeURIComponent(term)}`
        );
        ...
        suggest.innerHTML = items
          .map((it) => {
            const npsn = it.NPSN__c
              ? `<span class="muted">• NPSN ${it.NPSN__c}</span>`
              : "";
            return `<li class="suggest-item" data-id="${it.Id}" data-name="${(
              it.Name || ""
            ).replace(/"/g, "&quot;")}" data-npsn="${it.NPSN__c || ""}">${
              it.Name
            } ${npsn}</li>`;
          })
          .join("");
    ...
    if (!form.dataset.boundSubmit) {
      form.addEventListener("submit", async (e) => {
        ...
        if (!gradYear) {
          msg.textContent = "Pilih tahun lulus.";
          msg.style.display = "block";
          return;
        }
        if (!photo) {
          msg.textContent = "Pilih pas foto.";
          msg.style.display = "block";
          return;
        }
        ...
        if (manual) {
          ...
          if (idOk) {
            ...
            msg.textContent =
              'Pilih sekolah dari daftar autocomplete atau centang "Sekolah tidak ditemukan" untuk input manual.';
            msg.style.display = "block";
            input?.focus();
            return;
          }
        ...
        try {
          ...
          await api("/api/register-save-educ", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          });
          ...
          S.sekolah = {
            mode: sMode,
            schoolName,
            draftNpsn: payload.draftNpsn || null,
            gradYear,
    ...
    if (!form.dataset.boundClickShim) {
      form.addEventListener("click", (ev) => {
        ...
        if (backBtn && (ev.target === backBtn || backBtn.contains(ev.target)))
          return;

        const a = ev.target.closest("a");
        if (!a) return;
        const looksSubmit =
          a.classList.contains("submit-btn") ||
  ...
  function extractOppIdFromUrl() {
    ...
    if (
      parts.length >= 2 &&
      parts[0].toLowerCase() === "register" &&
      looksLikeSfId(parts[1])
    ) {
      return parts[1];
    }
  ...
  async function bootFromDeepLink() {
    ...
    try {
      ...
      const j = await api(
        `/api/register-status?opportunityId=${encodeURIComponent(oppId)}`
      );
